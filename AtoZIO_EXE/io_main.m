...
/*
 ...
 //Common function to allow ^T to toggle trace-mode in the middle of a test so that trace-files can be made smaller.
...
// fake a dynamically reconfigurable menu using the 0th entry to deselect the others
static bool
...
perform_menus(void)
{
  ...
  for (;;) {
    ...
    /// Provide for moving the menu around in the screen using shifted cursor keys.
    switch (ch) {
      case KEY_SF:
    ...
    switch (cmd) {
        /// The banner menu acts solely to select one of the other menus. Move between its items, wrapping at the left/right limits.
