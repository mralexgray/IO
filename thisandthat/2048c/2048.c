...
bool moveUp(uint8_t board[SIZE][SIZE]) {
	bool success = false;
	uint8_t x;
	for (x=0;x<SIZE;x++) success |= slideArray(board[x]);
	return success;
}
...
bool findPairDown(uint8_t board[SIZE][SIZE]) {
	bool success = false;
	uint8_t x,y;
	for (x=0;x<SIZE;x++) for (y=0;y<SIZE-1;y++) if (board[x][y]==board[x][y+1]) return true;
	return success;
}
...
int main(int argc, char *argv[]) {
	...
	if (argc == 2) {
    if (!strcmp(argv[1],"test")) return test();
    if (!strcmp(argv[1],"blackwhite")) scheme = 1;
    if (!strcmp(argv[1],"bluered"))    scheme = 2;
  }
